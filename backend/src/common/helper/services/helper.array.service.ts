...
export class HelperArrayService implements IHelperArrayService {
    getLeftByIndex<T>(array: T[], index: number): T | undefined {
        return _.nth(array, index);
    }

    getRightByIndex<T>(array: T[], index: number): T | undefined {
    ...
    getFirst<T>(array: T[]): T | undefined {
        return _.head(array);
    }
    ...
    differenceBy<T extends Record<string, any>>(a: T[], b: T[], property: keyof T): T[] {
        return _.differenceBy(a, b, (item) => item[property]);
    }
}
